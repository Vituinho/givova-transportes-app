...
export default function AboutScreen() {
  return <Screen><Header title="Sobre a Givova" back /><View style={styles.hero}><Text style={styles.eyebrow}>Givova Transportes</Text><Text style={styles.title}>Logística com proximidade e responsabilidade.</Text><Text style={styles.description}>Uma apresentação objetiva da empresa, preparada para receber conteúdo institucional validado pela Givova.</Text></View><View style={styles.list}>{topics.map(({ title, text, icon: Icon }, index) => <View key={title} style={[styles.item, index < topics.length - 1 && styles.divider]}><View style={styles.icon}><Icon size={22} color={colors.brandOrangeDark} /></View><View style={styles.copy}><Text style={styles.itemTitle}>{title}</Text><Text style={styles.itemText}>{text}</Text></View></View>)}</View><Text style={styles.disclaimer}>Antes da publicação, este conteúdo deve ser revisado pela equipe institucional para incluir apenas informações verificadas.</Text></Screen>;
}

const styles = StyleSheet.create({
  hero: { backgroundColor: colors.graphite, borderRadius: radius.lg, padding: 23, gap: 11 }, eyebrow: { color: colors.brandOrange, textTransform: 'uppercase', fontWeight: '800', letterSpacing: 1.2, fontSize: 12 }, title: { color: colors.white, fontSize: 29, lineHeight: 35, fontWeight: '900' }, description: { color: '#D6D6D6', fontSize: 14, lineHeight: 21 }, list: { borderTopWidth: 1, borderBottomWidth: 1, borderColor: colors.border }, item: { minHeight: 92, flexDirection: 'row', alignItems: 'center', paddingVertical: 14, gap: 14 }, divider: { borderBottomWidth: StyleSheet.hairlineWidth, borderBottomColor: colors.border }, copy: { flex: 1 }, icon: { width: 42, height: 42, borderRadius: radius.md, backgroundColor: colors.mutedOrange, alignItems: 'center', justifyContent: 'center' }, itemTitle: { color: colors.textPrimary, fontSize: 16, fontWeight: '800' }, itemText: { color: colors.textSecondary, fontSize: 13, lineHeight: 19, marginTop: 3 }, disclaimer: { color: colors.textSecondary, fontSize: 11, lineHeight: 17, textAlign: 'center' },
});
