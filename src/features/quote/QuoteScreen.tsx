...
const steps = ['Rota', 'Carga', 'Contato', 'Revisar'] as const;
  ...
  return (
    <FormProvider {...form}>
      <Screen keyboard contentContainerStyle={styles.screen}>
        ...
        <View style={styles.actions}>{step > 0 ? <Button label="Voltar" variant="outline" fullWidth={false} onPress={back} icon={<ChevronLeft size={18} color={colors.textPrimary} />} /> : null}<View style={styles.actionPrimary}>{step < steps.length - 1 ? <Button label="Continuar" onPress={next} icon={<ChevronRight size={18} color={colors.white} />} /> : <Button label="Solicitar cotação" onPress={form.handleSubmit((data) => quote.mutate(data))} loading={quote.isPending} icon={<ClipboardCheck size={19} color={colors.white} />} />}</View></View>
        <View style={styles.safety}><PackagePlus size={17} color={colors.textSecondary} /><Text style={styles.safetyText}>A cotação só é enviada na última etapa.</Text></View>
      </Screen>
    </FormProvider>
  );
}
...
const styles = StyleSheet.create({
  screen: { paddingBottom: 176, gap: 20 }, progressWrap: { gap: 10 }, progressTop: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }, progressSegments: { flexDirection: 'row', gap: 6 }, progressSegment: { flex: 1, height: 4, borderRadius: 2, backgroundColor: colors.border }, progressSegmentActive: { backgroundColor: colors.brandOrange }, progressText: { color: colors.textSecondary, fontSize: 12, fontWeight: '600' }, progressName: { color: colors.brandOrangeDark, fontSize: 12, fontWeight: '800' }, form: { paddingVertical: 2 }, fields: { gap: 14 }, stepTitle: { color: colors.textPrimary, fontSize: 23, fontWeight: '900', letterSpacing: -0.4 }, stepDescription: { color: colors.textSecondary, fontSize: 14, lineHeight: 20, marginTop: -7 }, twoColumns: { flexDirection: 'row', gap: 10 }, column: { flex: 1 }, routeDivider: { flexDirection: 'row', alignItems: 'center', gap: 10, marginVertical: 1 }, routeLine: { height: 1, backgroundColor: colors.border, flex: 1 }, routeDividerText: { color: colors.textSecondary, fontSize: 11, textTransform: 'uppercase', letterSpacing: 1 }, summaryCard: { gap: 9, padding: 14, backgroundColor: colors.white }, summaryTitle: { color: colors.brandOrangeDark, fontSize: 13, fontWeight: '800', textTransform: 'uppercase', letterSpacing: 0.7 }, summaryRow: { flexDirection: 'row', gap: 12 }, summaryLabel: { width: 92, color: colors.textSecondary, fontSize: 12 }, summaryValue: { flex: 1, color: colors.textPrimary, fontSize: 13, fontWeight: '600', textAlign: 'right' }, checkboxRow: { minHeight: 54, flexDirection: 'row', alignItems: 'center', gap: 11 }, checkbox: { width: 24, height: 24, borderRadius: 6, borderWidth: 2, borderColor: colors.border, backgroundColor: colors.white, alignItems: 'center', justifyContent: 'center' }, checkboxChecked: { backgroundColor: colors.brandOrange, borderColor: colors.brandOrange }, checkboxText: { color: colors.textPrimary, flex: 1, fontSize: 13, lineHeight: 19 }, link: { color: colors.brandOrangeDark, fontWeight: '800' }, privacyError: { color: colors.error, fontSize: 12, marginLeft: 35 }, actions: { flexDirection: 'row', alignItems: 'center', gap: 10 }, actionPrimary: { flex: 1 }, safety: { flexDirection: 'row', justifyContent: 'center', gap: 7 }, safetyText: { color: colors.textSecondary, fontSize: 12 }, success: { flex: 1, alignItems: 'center', justifyContent: 'center', gap: 15, paddingHorizontal: 8 }, successIcon: { width: 78, height: 78, borderRadius: 39, backgroundColor: colors.mutedSuccess, alignItems: 'center', justifyContent: 'center' }, successTitle: { color: colors.textPrimary, fontSize: 28, fontWeight: '900', textAlign: 'center' }, successText: { color: colors.textSecondary, fontSize: 15, lineHeight: 22, textAlign: 'center', marginBottom: 6 }, protocol: { width: '100%', padding: 14, backgroundColor: colors.background, borderRadius: radius.md, alignItems: 'center', marginBottom: 7 }, protocolLabel: { color: colors.textSecondary, fontSize: 11, textTransform: 'uppercase' }, protocolValue: { color: colors.textPrimary, fontSize: 18, fontWeight: '800', marginTop: 3 },
});
